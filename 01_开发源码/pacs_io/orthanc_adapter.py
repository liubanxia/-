...
class OrthancAdapter(PacsAdapter):
    ...
    def load_case(self, case_ref):
        ...
        self.temp_dir = Path(
            tempfile.mkdtemp(prefix="phoenix_pacs_")
        )

        instances = self._json(
            f"/studies/{case_ref}/instances"
        )
        ...
        for i, item in enumerate(instances):
            ...
            with urlopen(
                self.url + f"/instances/{instance_id}/file"
            ) as r:
                data = r.read()

            path = self.temp_dir / f"{i:06d}.dcm"
            path.write_bytes(data)

        case = FolderPacsAdapter().load_case(
            str(self.temp_dir)
        )
