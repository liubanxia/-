...
def _label(item: Dict[str, Any]) -> str:
    # Prefer human-readable model finding over numeric class ids.
...
def _normalise_lesion(model_name: str, item: Dict[str, Any]) -> Lesion:
    ...
    box = _tuple_or_none(
        ...
        else geometry.get("box")
    )

    box_3d = _tuple_or_none(
        item.get("box_3d")
        if item.get("box_3d") is not None
        else geometry.get("box_3d")
    )

    world_point_lps = _tuple_or_none(item.get("world_point_lps"))
    geometry_mode = str(item.get("geometry_mode", "") or "")

    # MONAI lung_nodule_ct_detection postprocessing is configured as:
    # AffineBoxToWorldCoordinated -> ConvertBoxMode xyzxyz -> cccwhd.
    # Phoenix keeps affine_lps_to_ras=False, so the first three values are
    # DICOM-patient LPS world coordinates of the box centre.
    if (
        model_name == "monai_lung_nodule_ct"
        and box_3d is not None
        and len(box_3d) >= 6
    ):
        world_point_lps = (
            float(box_3d[0]),
        ...
        geometry_mode = geometry_mode or "cccwhd_lps"
    ...
    return Lesion(
        ...
        box_3d=box_3d,
        world_point_lps=(
            tuple(world_point_lps[:3])  # type: ignore[arg-type]
            if world_point_lps is not None and len(world_point_lps) >= 3
            else None
        ),
        ...
        for item in lesions:
            ...
            try:
                result.lesions.append(
                    _normalise_lesion(model_name, item)
                )
            except Exception as exc:
                result.warnings.append(
                    f"{model_name}病灶结果解析失败: {type(exc).__name__}: {exc}"
                )
