...
class ReportTeacherRuntime:
    ...
    def load(self, device="cpu"):
        ...
        path = self._path()
        try:
            self.processor = transformers.AutoProcessor.from_pretrained(path, local_files_only=True, trust_remote_code=True)
        except Exception:
            self.processor = None
        try:
            self.tokenizer = transformers.AutoTokenizer.from_pretrained(path, local_files_only=True, trust_remote_code=True)
        except Exception:
            self.tokenizer = None
        classes = [
        ...
        for cls in classes:
            if cls is None:
                continue
            try:
                self.model = cls.from_pretrained(path, local_files_only=True, trust_remote_code=True, low_cpu_mem_usage=False, device_map=None)
                break
    ...
    def generate(self, prompt, max_new_tokens=512):
        ...
        try:
            inputs = frontend(text=prompt, return_tensors="pt")
        except TypeError:
            inputs = frontend(prompt, return_tensors="pt")
        device = next(self.model.parameters()).device
        inputs = {k: v.to(device) if torch.is_tensor(v) else v for k, v in inputs.items()}
        with torch.inference_mode():
            ids = self.model.generate(**inputs, max_new_tokens=max_new_tokens, do_sample=False)
        decoder = self.tokenizer or getattr(self.processor, "tokenizer", None)
        if decoder is None:
