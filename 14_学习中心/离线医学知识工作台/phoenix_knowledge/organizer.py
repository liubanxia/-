...
class DeepOrganizer:
    def __init__(
        ...
        self.assets = PDFAssetStore(runtime_root or (self.evidence_root.parent / "_runtime"))
...
"""
    ...
    @staticmethod
    def _evidence_pack(title: str, instruction: str, evidence: list[Evidence]) -> str:
        lines = [
            f"# {title}",
            "",
            f"> 整理要求：{instruction}",
    ...
    def _attach_images(
        ...
        *,
        ...
        """Copy source-page figures beside the organized Markdown and link them."""
        selected = [
        ...
        for item in selected:
            ...
            try:
                copied = self.assets.copy_page_assets(
                    ...
                    item.page,
                    asset_dir,
                    prefix=f"{_safe_filename(item.title)[:36]}_",
                    ensure=True,
                )
            except Exception:
                copied = []
            ...
            if not copied:
                break
            blocks.extend(["", f"### {item.title} · 第{item.page}页", ""])
            for index, image_path in enumerate(copied, start=1):
                rel = f"{asset_dir.name}/{image_path.name}"
                blocks.extend([
                    f"![{item.title} 第{item.page}页 图{index}]({rel})",
                    "",
                ])
            image_count += len(copied)
            if image_count >= max_images:
                blocks.append(f"> 图片较多，本次整理最多附加 {max_images} 张相关原图；原始图片资料仍保存在本地图片库。")
                break
    ...
    def _load_task_context(self, task_id: int) -> tuple[object, dict, list[Evidence]]:
        ...
        if not evidence:
            raise RuntimeError("任务对应的PDF证据已经不存在，无法安全恢复。")
    ...
    def _hierarchical_merge(
        ...
        *,
        ...
        while len(level) > 1:
            merge_level += 1
            next_level: list[str] = []
            groups = [level[i:i + group_size] for i in range(0, len(level), group_size)]
            for group_index, group in enumerate(groups, start=1):
                ...
                if progress:
                    progress(
                        batch_total,
                        batch_total,
                        f"正在分层合并：第{merge_level}层 {group_index}/{len(groups)}",
                    )
    ...
    def resume(
        ...
        *,
        ...
        if status not in _RESUMABLE_STATUS:
            raise RuntimeError(f"任务 {task_id} 当前状态为 {status}，无需恢复。")
    ...
    def organize(
        ...
        *,
        ...
        title = title.strip() or instruction.strip()[:50] or "医学知识专题"
        ...
        if task_id is None:
            ...
            batch_size = max(1, int(payload.get("batch_size", batch_size) or batch_size))

        checkpoint = json.loads(task["checkpoint_json"] or "{}") if task else {}
        ...
        self.db.update_task(task_id, status="running", total=total_batches, error="")

        if not self.llm.available():
            text = self._evidence_pack(title, instruction, evidence)
            output = self.evidence_root / f"{_safe_filename(title)}.md"
            output.write_text(text, encoding="utf-8")
            self._attach_images(output, evidence)
            self.db.update_task(
                task_id,
                status="completed_evidence_only",
                progress=total_batches,
                checkpoint={"next_batch": total_batches, "partials": []},
            )
        ...
        try:
            for batch_index in range(next_batch, total_batches):
                ...
                if progress:
                    progress(
                        batch_index,
                        total_batches,
                        f"正在整理第 {batch_index + 1}/{total_batches} 批证据……",
                    )
                ...
                used = {int(x) for x in _CITATION_RE.findall(partial)} & valid_ids
                if not used:
                    partial = self._evidence_pack(
                        f"批次 {batch_index + 1}", instruction, batch
                    )
                partials.append(partial)
                self.db.update_task(
                    task_id,
                    checkpoint={"next_batch": batch_index + 1, "partials": partials},
            ...
            used_ids = {int(x) for x in _CITATION_RE.findall(final_text)} & valid_ids
            if not used_ids:
                final_text = "\n\n".join(partials)
            ...
            for item in evidence:
                if item.chunk_id in used_ids or not used_ids:
                    source_lines.append(f"- {item.citation} {item.title}，第{item.page}页")
            ...
            if progress:
                progress(total_batches, total_batches, "正文整理完成，正在附加相关原图……")
            image_count = self._attach_images(output, evidence, used_ids)
            if image_count:
                with output.open("a", encoding="utf-8") as handle:
                    handle.write(f"\n> 本专题已附加 {image_count} 张与引用页对应的PDF原图。\n")

            self.db.update_task(task_id, status="completed", progress=total_batches)
            self.db.record_output(title, instruction, output)
            return output, task_id
