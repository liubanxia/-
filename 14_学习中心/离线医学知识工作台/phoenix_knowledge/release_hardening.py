...
def _embedding_readiness(self) -> dict:
    ...
    if not model_ready:
        ...
        state, label = "runtime_missing", "语义组件缺失"
    elif chunks == 0:
...
def _audit_has_hard_failure(payload: dict) -> bool:
    return any(str(part.get("backend", "")).strip() == "failed_all" for part in (payload.get("parts") or ()))
...
def _prepare_hard_failed_pages(translator, pdf_path: Path, target_language: str) -> int:
    try:
        ...
        _, pages_root, audit_root, _, _ = translator._book_paths(source, digest, target_language)
    except Exception:
        return 0
    removed = 0
...
def _safe_translation_pdf_build(self, *, start_page: int, total_pages: int, layout: str, part_pages: int = 50, progress=None):
    import fitz
    from .translation_pdf import LAYOUT_ORIGINAL_BILINGUAL, LAYOUT_TEXT_BILINGUAL, LAYOUT_TRANSLATED_ONLY
    layout = layout if layout in {LAYOUT_ORIGINAL_BILINGUAL, LAYOUT_TEXT_BILINGUAL, LAYOUT_TRANSLATED_ONLY} else LAYOUT_ORIGINAL_BILINGUAL
    part_pages = max(1, int(part_pages))
    start_page = max(1, int(start_page))
    total_pages = max(start_page, int(total_pages))
    complete_name = (
        "完整双语译本_原页在上中文在下.pdf" if layout == LAYOUT_ORIGINAL_BILINGUAL
        else "完整双语译本_英文在上中文在下.pdf" if layout == LAYOUT_TEXT_BILINGUAL
        else "完整中文译本.pdf"
    )
    ...
    try:
        for offset, page_number in enumerate(range(start_page, total_pages + 1), start=1):
            ...
            if progress:
                progress(offset, selected_total, f"正在生成PDF页面：第 {page_number}/{total_pages} 页")
        if progress:
            progress(selected_total, selected_total, "页面已生成，正在压缩写入完整PDF；此阶段不会重新翻译。")
    ...
    try:
        ...
        for part_index, start_index in enumerate(range(0, total_output_pages, part_pages), start=1):
            ...
            part_path = parts_root / f"第{part_index:03d}册_{first_source_page:04d}-{last_source_page:04d}.pdf"
            part_doc = fitz.open()
            try:
                part_doc.insert_pdf(complete, from_page=start_index, to_page=end_index)
                if progress:
                    progress(selected_total, selected_total, f"正在写入PDF分册 {part_index}/{part_total}：{first_source_page}-{last_source_page} 页")
    ...
    if progress:
        progress(selected_total, selected_total, f"PDF成品写入完成：完整PDF + {len(part_paths)} 册分册。")
...
def install() -> None:
    ...
    def ingest(self, path: Path, **kwargs):
        ...
        semantic = self.retriever.embeddings.readiness()
        if semantic["model_ready"] and semantic["runtime_ready"] and semantic["missing"] > 0:
            if progress:
                progress(0, max(1, semantic["missing"]), "资料索引完成，正在自动补齐语义向量……")
    ...
    def available_backends(self):
        ...
        return [name for name in names if not (_commercial_release(self.paths) and name == self.nllb.name)]
    def active_backends(self, target_language="中文", smart_level="smart1"):
        backends = list(original_active_backends(self, target_language, smart_level))
        return [backend for backend in backends if not (_commercial_release(self.paths) and getattr(backend, "name", "") == self.nllb.name)]
    ...
    def translate_book(self, pdf_path: Path, **kwargs):
        ...
        hard_failed = _prepare_hard_failed_pages(self, Path(pdf_path), target_language)
        if hard_failed and kwargs.get("progress"):
            kwargs["progress"](0, 1, f"检测到 {hard_failed} 个硬失败页，将自动重新翻译，不再错误跳过。")
