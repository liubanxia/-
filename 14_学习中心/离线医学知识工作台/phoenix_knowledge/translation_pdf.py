...
class TranslationPDFBuilder:
    ...
    def _estimated_text_height(
        ...
        """Estimate layout using CJK glyph width instead of raw char count.

        Chinese glyphs are roughly full-em wide while Latin prose averages
        close to half-em. The previous all-Latin estimate made long Chinese
        pages too short and forced insert_htmlbox to shrink aggressively.
        """
        ...
        return max(
            220.0,
            logical_lines * font_size * 1.72 + 110.0,
        )
    ...
    def _append_text_bilingual_page(
        ...
        source_rect = page.rect.__class__(
            26,
            24,
            width - 26,
            24 + source_height,
        ...
        *,
        ...
        try:
            ...
            for offset, page_number in enumerate(
                ...
                if (
                    source_index < 0
                    or source_index >= source_doc.page_count
                ):
                    raise RuntimeError(
                        f"源PDF不存在第 {page_number} 页"
                    )
                if layout == LAYOUT_ORIGINAL_BILINGUAL:
