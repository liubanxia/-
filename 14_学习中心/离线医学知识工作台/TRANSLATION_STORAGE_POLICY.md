...
- PDF writes are transactional and lossless. The complete PDF, optional split-volume directory, size report and integrity report are published as one rollback set only after staging validation; a post-publish reopen failure restores the last known-good set. Normal writes use `garbage=2`, Flate compression, font/image stream compression and object streams. A `garbage=3` duplicate-object pass is attempted only when the first output exceeds the storage budget; `garbage=4` is forbidden for whole-book translation because large stream scanning caused the old 383-page final-save stall.
- Storage budget for the complete translated PDF is a hard release gate: `max(source × 1.18, source + 2.5 MiB)`. The fixed allowance covers the CJK text/font layer on small PDFs. Over-budget staging files are rejected before an existing stable output is touched. Larger medical books should normally remain close to 1.0×.
