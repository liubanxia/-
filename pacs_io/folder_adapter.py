...
def _read_header(path: Path):
    ...
    tags = [
        ...
        "SeriesDescription", "ProtocolName", "ImagePositionPatient",
        "ImageOrientationPatient", "SliceLocation", "InstanceNumber",
    ]
    try:
        return pydicom.dcmread(str(path), stop_before_pixels=True, force=False, specific_tags=tags)
    except Exception:
        if path.suffix.lower() not in _DICOM_EXTENSIONS:
            raise
        return pydicom.dcmread(str(path), stop_before_pixels=True, force=True, specific_tags=tags)
...
class FolderPacsAdapter(PacsAdapter):
    def load_case(self, case_ref: str) -> CaseInput:
        ...
        for path in root.rglob("*"):
            ...
            grouped[(study_uid, series_uid)].append((_projection_sort_value(ds), path, modality, description, protocol))
        ...
        if len(study_counts) > 1:
            warnings.append(f"目录包含多个StudyInstanceUID；仅选择影像数量最多的Study: {selected_study_uid}。")

        series = []
        for (study_uid, series_uid), entries in grouped.items():
            if study_uid != selected_study_uid:
                continue
            entries.sort(key=lambda item: (item[0][0], item[0][1], str(item[1])))
            first = entries[0]
        ...
        series.sort(key=lambda item: (str(item.modality), str(item.series_description), str(item.series_uid)))
        return CaseInput(
