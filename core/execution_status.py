...
def execution_from_raw(model_name: str, raw: Any, load_status: str = "", load_error: str = "") -> ModelExecution:
    ...
    execution.metadata = {k: v for k, v in raw.items() if k not in {"lesions", "error"}}
    return execution
...
def summarize_executions(executions: Iterable[ModelExecution]) -> Dict[str, Any]:
    ...
    return {
        ...
        "failed_models": [x.model_name for x in items if x.status == "failed"],
